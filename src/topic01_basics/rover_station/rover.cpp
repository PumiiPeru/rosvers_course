...
{
	// Initiate new ROS node named "rover"
	ros::init(argc, argv, "rover_node");

	//create a node handle: it is reference assigned to a new node
	ros::NodeHandle n;
	//create a publisher with a topic "position" that will send a String message
	ros::Publisher position_publisher = n.advertise<std_msgs::String>("position", 1000);
